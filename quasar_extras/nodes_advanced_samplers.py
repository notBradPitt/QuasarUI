...
def sample_lcm_upscale(model, x, sigmas, extra_args=None, callback=None, disable=None, total_upscale=2.0, upscale_method="bislerp", upscale_steps=None):
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        x = denoised
        if i < len(upscales):
            x = quasar.utils.common_upscale(x, round(orig_shape[-1] * upscales[i]), round(orig_shape[-2] * upscales[i]), upscale_method, "disabled")
...
class SamplerLCMUpscale:
    ...
    def get_sampler(self, scale_ratio, scale_steps, upscale_method):
        if scale_steps < 0:
            scale_steps = None
        sampler = quasar.samplers.KSAMPLER(sample_lcm_upscale, extra_options={"total_upscale": scale_ratio, "upscale_steps": scale_steps, "upscale_method": upscale_method})
        return (sampler, )

from quasar.k_diffusion.sampling import to_d
...
def sample_euler_pp(model, x, sigmas, extra_args=None, callback=None, disable=None):
    ...
    extra_args["model_options"] = quasar.model_patcher.set_model_options_post_cfg_function(model_options, post_cfg_function, disable_cfg1_optimization=True)
...
class SamplerEulerCFGpp:
    ...
    def get_sampler(self, version):
        if version == "alternative":
            sampler = quasar.samplers.KSAMPLER(sample_euler_pp)
        else:
            sampler = quasar.samplers.ksampler("euler_cfg_pp")
        return (sampler, )
