...
class PerpNeg:
    ...
    def patch(self, model, empty_conditioning, neg_scale):
        m = model.clone()
        nocond = quasar.sampler_helpers.convert_cond(empty_conditioning)
        ...
        def cfg_function(args):
            ...
            nocond_processed = quasar.samplers.encode_model_conds(model.extra_conds, nocond, x, x.device, "negative")

            (noise_pred_nocond,) = quasar.samplers.calc_cond_batch(model, [nocond_processed], x, sigma, model_options)
...
class Guider_PerpNeg(quasar.samplers.CFGGuider):
    ...
    def predict_noise(self, x, timestep, model_options={}, seed=None):
        ...
        (noise_pred_pos, noise_pred_neg, noise_pred_empty) = \
            quasar.samplers.calc_cond_batch(self.inner_model, [positive_cond, negative_cond, empty_cond], x, timestep, model_options)
