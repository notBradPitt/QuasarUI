...
class BetaSamplingScheduler:
    ...
    def get_sigmas(self, model, steps, alpha, beta):
        sigmas = quasar.samplers.beta_scheduler(model.get_model_object("model_sampling"), steps, alpha=alpha, beta=beta)
        return (sigmas, )
...
class KSamplerSelect:
    ...
    def INPUT_TYPES(s):
        return {"required":
                    {"sampler_name": (quasar.samplers.SAMPLER_NAMES, ),
                      }
               }
...
class SamplerDPMPP_2S_Ancestral:
    ...
    def get_sampler(self, eta, s_noise):
        sampler = quasar.samplers.ksampler("dpmpp_2s_ancestral", {"eta": eta, "s_noise": s_noise})
        return (sampler, )
...
class SamplerCustomAdvanced:
    ...
    def sample(self, noise, guider, sampler, sigmas, latent_image):
        ...
        latent_image = latent["samples"]
        latent = latent.copy()
        latent_image = quasar.sample.fix_empty_latent_channels(guider.model_patcher, latent_image)
        latent["samples"] = latent_image
