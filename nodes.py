...
class VAELoader:
    ...
    def load_taesd(name):
        ...
        enc = quasar.utils.load_torch_file(folder_paths.get_full_path("vae_approx", encoder))
        for k in enc:
            sd["taesd_encoder.{}".format(k)] = enc[k]

        dec = quasar.utils.load_torch_file(folder_paths.get_full_path("vae_approx", decoder))
        for k in dec:
...
class UNETLoader:
    ...
    def load_unet(self, unet_name, weight_dtype):
        ...
        model = quasar.sd.load_diffusion_model(unet_path, model_options=model_options)
        return (model,)
...
class CLIPLoader:
    ...
    def load_clip(self, clip_name, type="stable_diffusion"):
        if type == "stable_cascade":
            clip_type = quasar.sd.CLIPType.STABLE_CASCADE
        elif type == "sd3":
            clip_type = quasar.sd.CLIPType.SD3
        elif type == "stable_audio":
            clip_type = quasar.sd.CLIPType.STABLE_AUDIO
        else:
            clip_type = quasar.sd.CLIPType.STABLE_DIFFUSION

        clip_path = folder_paths.get_full_path("clip", clip_name)
        clip = quasar.sd.load_clip(ckpt_paths=[clip_path], embedding_directory=folder_paths.get_folder_paths("embeddings"), clip_type=clip_type)
        return (clip,)
...
class DualCLIPLoader:
    ...
    def load_clip(self, clip_name1, clip_name2, type):
        ...
        if type == "sdxl":
            ...
            clip_type = quasar.sd.CLIPType.FLUX

        clip = quasar.sd.load_clip(ckpt_paths=[clip_path1, clip_path2], embedding_directory=folder_paths.get_folder_paths("embeddings"), clip_type=clip_type)
        return (clip,)
...
class EmptyLatentImage:
    def __init__(self):
        self.device = quasar.model_management.intermediate_device()
...
def common_ksampler(model, seed, steps, cfg, sampler_name, scheduler, positive, negative, latent, denoise=1.0, disable_noise=False, start_step=None, last_step=None, force_full_denoise=False):
    ...
    latent_image = quasar.sample.fix_empty_latent_channels(model, latent_image)
...
def get_module_name(module_path: str) -> str:
    ...
    Examples:
        get_module_name("C:/Users/username/QuasarUI/custom_nodes/my_custom_node.py") -> "my_custom_node"
        get_module_name("C:/Users/username/QuasarUI/custom_nodes/my_custom_node") -> "my_custom_node"
        get_module_name("C:/Users/username/QuasarUI/custom_nodes/my_custom_node/") -> "my_custom_node"
        get_module_name("C:/Users/username/QuasarUI/custom_nodes/my_custom_node/__init__.py") -> "my_custom_node"
        get_module_name("C:/Users/username/QuasarUI/custom_nodes/my_custom_node/__init__") -> "my_custom_node"
        get_module_name("C:/Users/username/QuasarUI/custom_nodes/my_custom_node/__init__/") -> "my_custom_node"
        get_module_name("C:/Users/username/QuasarUI/custom_nodes/my_custom_node.disabled") -> "custom_nodes
    Args:
...
def init_builtin_extra_nodes():
    """
    Initializes the built-in extra nodes in QuasarUI.

    This function loads the extra node files located in the "quasar_extras" directory and imports them into QuasarUI.
    If any of the extra node files fail to import, a warning message is logged.

    Returns:
        None
    """
    extras_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "quasar_extras")
    extras_files = [
        "nodes_latent.py",
    ...
    for node_file in extras_files:
        if not load_custom_node(os.path.join(extras_dir, node_file), module_parent="quasar_extras"):
...
def init_extra_nodes(init_custom_nodes=True):
    ...
    if len(import_failed) > 0:
        logging.warning("WARNING: some quasar_extras/ nodes did not import correctly. This may be because they are missing some dependencies.\n")
        for node in import_failed:
            logging.warning("IMPORT FAILED: {}".format(node))
        logging.warning("\nThis issue might be caused by new missing dependencies added the last time you updated QuasarUI.")
        if args.windows_standalone_build:
            logging.warning("Please run the update script: update/update_quasarui.bat")
        else:
