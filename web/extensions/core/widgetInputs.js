import { QuasarWidgets, addValueControlWidgets } from "../../scripts/widgets.js";
...
app.registerExtension({
	name: "Quasar.WidgetInputs",
	init() {
		useConversionSubmenusSetting = app.ui.settings.addSetting({
			id: "Quasar.NodeInputConversionSubmenus",
	...
	async beforeRegisterNodeDef(nodeType, nodeData, app) {
		...
		nodeType.prototype.onInputDblClick = function (slot) {
			...
			if (!input.widget || !input[ignoreDblClick]) {
				...
				if (!(input.type in QuasarWidgets) && !(input.widget[GET_CONFIG]?.()?.[0] instanceof Array)) {
					return r; //also Not a QuasarWidgets input or combo (do nothing)
				}
			}
	...
	registerCustomNodes() {
		...
		class PrimitiveNode {
			...
			onConnectOutput(slot, type, input, target_node, target_slot) {
				...
				if (!input.widget) {
					if (!(input.type in QuasarWidgets)) return false;
				}
			...
			#onFirstConnection(recreating) {
				...
				let widget;
				if (!input.widget) {
					if (!(input.type in QuasarWidgets)) return;
			...
			#createWidget(inputData, node, widgetName, recreating, targetWidget) {
				...
				let widget;
				if (type in QuasarWidgets) {
					widget = (QuasarWidgets[type](this, "value", inputData, app) || {}).widget;
				} else {
