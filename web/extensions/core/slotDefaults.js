...
app.registerExtension({
	name: "Quasar.SlotDefaults",
	suggestionsNumber: null,
	init() {
		LiteGraph.search_filter_enabled = true;
		LiteGraph.middle_click_slot_add_default_node = true;
		this.suggestionsNumber = app.ui.settings.addSetting({
			id: "Quasar.NodeSuggestions.number",
	...
	async beforeRegisterNodeDef(nodeType, nodeData, app) {
		...
		for (const inputKey in inputs) {
			...
			LiteGraph.registered_slot_in_types[lowerType].nodes.push(nodeType.quasarClass);
		} 
		...
		for (const key in outputs) {
			...
			LiteGraph.registered_slot_out_types[type].nodes.push(nodeType.quasarClass);
