...
async function uploadMask(filepath, formData) {
	...
	QuasarApp.clipspace.imgs[QuasarApp.clipspace['selectedIndex']] = new Image();
	QuasarApp.clipspace.imgs[QuasarApp.clipspace['selectedIndex']].src = api.apiURL("/view?" + new URLSearchParams(filepath).toString() + app.getPreviewFormatParam() + app.getRandParam());

	if(QuasarApp.clipspace.images)
		QuasarApp.clipspace.images[QuasarApp.clipspace['selectedIndex']] = filepath;
...
class MaskEditorDialog extends QuasarDialog {
	...
	createLeftSlider(self, name, callback) {
		...
		divElement.style.backgroundColor = "var(--quasar-input-bg)";
			...
			mutations.forEach(function(mutation) {
					if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
						if(self.last_display_style && self.last_display_style != 'none' && self.element.style.display == 'none') {
							...
							QuasarApp.onClipspaceEditorClosed();
						}
	...
	async setImages(imgCanvas) {
		...
		// image load
		const filepath = QuasarApp.clipspace.images;

		const alpha_url = new URL(QuasarApp.clipspace.imgs[QuasarApp.clipspace['selectedIndex']].src)
		...
		// original image load
		const rgb_url = new URL(QuasarApp.clipspace.imgs[QuasarApp.clipspace['selectedIndex']].src);
	...
	async save() {
		...
		if(QuasarApp.clipspace.images)
			QuasarApp.clipspace.images[0] = item;

		if(QuasarApp.clipspace.widgets) {
			const index = QuasarApp.clipspace.widgets.findIndex(obj => obj.name === 'image');

			if(index >= 0)
				QuasarApp.clipspace.widgets[index].value = item;
		}
		...
		QuasarApp.onClipspaceEditorSave();
		this.close();
	}
}

app.registerExtension({
	name: "Quasar.MaskEditor",
	init(app) {
		...
		const context_predicate = () => QuasarApp.clipspace && QuasarApp.clipspace.imgs && QuasarApp.clipspace.imgs.length > 0
		ClipspaceDialog.registerButton("MaskEditor", context_predicate, QuasarApp.open_maskeditor);
	}
});
