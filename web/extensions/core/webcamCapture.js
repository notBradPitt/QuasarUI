...
app.registerExtension({
				...
				const loadVideo = async () => {
					try {
						...
						if (window.isSecureContext) {
							...
							label.textContent = "Unable to load webcam. A secure context is required, if you are not accessing QuasarUI on localhost (127.0.0.1) you will have to enable TLS (https)\n\n" + error.message;
						}
	...
	nodeCreated(node) {
		if ((node.type, node.constructor.quasarClass !== "WebcamCapture")) return;
