...
export class QuasarViewListButton {
	get open() {
	...
	constructor(app, { button, list, mode }) {
		this.app = app;
		this.button = button;
		this.element = $el("div.quasarui-button-wrapper", this.button.element);
		this.popup = new QuasarPopup({
			target: this.element,
			container: this.element,
			horizontal: "right",
		});
		this.list = new (list ?? QuasarViewList)(app, mode, this.popup);
...
export class QuasarViewList {
	...
	constructor(app, mode, popup) {
		...
		this.items = $el(`div.quasarui-${this.type}-items.quasarui-view-list-items`);
		this.clear = new QuasarButton({
			icon: "cancel",
			content: "Clear",
			action: async () => {
		...
		this.element = $el(`div.quasarui-${this.type}-popup.quasarui-view-list-popup`, [
			$el("h3", mode),
