...
import { QuasarPopup } from "../components/popup.js";
import { createSpinner } from "../spinner.js";
import { QuasarWorkflow, trimJsonExt } from "../../workflows.js";
import { QuasarAsyncDialog } from "../components/asyncDialog.js";

export class QuasarWorkflowsMenu {
	#first = true;
	element = $el("div.quasarui-workflows");
	...
	constructor(app) {
		...
		const classList = {
			"quasarui-workflows-button": true,
			"quasarui-button": true,
			unsaved: getStorageValue("Quasar.PreviousWorkflowUnsaved") === "true",
			running: false,
		};
		this.buttonProgress = $el("div.quasarui-workflows-button-progress");
		this.workflowLabel = $el("span.quasarui-workflows-label", "");
		this.button = new QuasarButton({
			content: $el("div.quasarui-workflows-button-inner", [$el("i.mdi.mdi-graph"), this.workflowLabel, this.buttonProgress]),
			icon: "chevron-down",
			classList,
		});
		...
		this.popup = new QuasarPopup({ target: this.element, classList: "quasarui-workflows-popup" });
		this.content = new QuasarWorkflowsContent(app, this.popup);
		...
		this.unsaved = prop(this, "unsaved", classList.unsaved, (v) => {
			...
			setStorageValue("Quasar.PreviousWorkflowUnsaved", v);
		});
	}
		...
		app.registerExtension({
			...
			async beforeRegisterNodeDef(nodeType) {
				...
				nodeType.prototype["getExtraMenuOptions"] = function (_, options) {
					...
					const setting = app.ui.settings.getSettingValue("Quasar.UseNewMenu", false);
...
export class QuasarWorkflowsContent {
	element = $el("div.quasarui-workflows-panel");
	treeState = {};
	treeFiles = {};
	/** @type { Map<QuasarWorkflow, WorkflowElement> } */
	openFiles = new Map();
	...
	/**
	 * @param {import("../../app.js").QuasarApp} app
	 * @param {QuasarPopup} popup
	 */
	constructor(app, popup) {
		this.app = app;
		this.popup = popup;
		this.actions = $el("div.quasarui-workflows-actions", [
			new QuasarButton({
				content: "Default",
				icon: "file-code",
				iconSize: 18,
				classList: "quasarui-button primary",
	...
	updateOpen() {
		...
		this.openElement = $el("div.quasarui-workflows-open", [
			$el("h3", "Open"),
				...
				const wrapper = new WorkflowElement(this, w, {
					...
					buttons: [
						...
						new QuasarButton({
							icon: "close",
							iconSize: 18,
							classList: "quasarui-button quasarui-workflows-file-action",
	...
	updateFavorites() {
		...
		this.favoritesElement = $el("div.quasarui-workflows-favorites", [
			$el("h3", "Favorites"),
			...favorites
				.map((w) => {
	...
	hideTreeParents(element) {
		...
		if (element.parentElement?.classList.contains("quasarui-workflows-tree") === false) {
	...
	updateTree() {
		...
		this.treeFiles = {};
		this.treeRoot = $el("ul.quasarui-workflows-tree");
		this.treeElement = $el("section", [
			$el("header", [
				$el("h3", "Browse"),
				$el("div.quasar-ui-workflows-search", [
	...
	#getFavoriteButton(workflow, primary) {
		return new QuasarButton({
			icon: this.#getFavoriteIcon(workflow),
			overIcon: this.#getFavoriteOverIcon(workflow),
			iconSize: 18,
			classList: "quasarui-button quasarui-workflows-file-action-favorite" + (primary ? " quasarui-workflows-file-action-primary" : ""),
	...
	#getDeleteButton(workflow) {
		const deleteButton = new QuasarButton({
			icon: "delete",
...
class WorkflowElement {
	/**
	 * @param { QuasarWorkflowsContent } parent
	 * @param { QuasarWorkflow } workflow
	 */
	constructor(parent, workflow, { tagName = "li", primary, buttons }) {
		...
		this.element = $el(
			tagName + ".quasarui-workflows-tree-file",
			{
				onclick: () => {
					workflow.load();
...
class WidgetSelectionDialog extends QuasarAsyncDialog {
	#options;
	...
	show(app) {
		this.element.classList.add("quasar-widget-selection-dialog");
		return super.show(
			$el("div", [
				...
				$el(
					...
					this.#options.map((opt) => {
						return $el("div.quasar-widget-selection-item", [
							...
							$el(
								"button.quasarui-button.primary",
								{
									onclick: () => {
										this.close(opt);
									},
