...
export function createImageHost(node) {
	const el = $el("div.quasar-img-preview");
	let currentImgs;
	let first = true;
	...
	function updateSize() {
		...
		if (currentImgs) {
			...
			if (first) {
				...
				el.style.setProperty("--quasar-widget-min-height", elH);
			} else {
				el.style.setProperty("--quasar-widget-min-height", null);
			}
			...
			el.style.setProperty("--quasar-img-preview-width", w);
			el.style.setProperty("--quasar-img-preview-height", h);
		}
	}
	return {
		el,
