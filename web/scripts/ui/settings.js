...
import { QuasarDialog } from "./dialog.js";

export class QuasarSettingsDialog extends QuasarDialog {
	constructor(app) {
		super();
		this.app = app;
		this.settingsValues = {};
	...
	async setSettingValueAsync(id, value) {
		...
		localStorage["Quasar.Settings." + id] = json; // backwards compatibility for extensions keep setting in storage
	...
	addSetting({ id, name, type, defaultValue, onChange, attrs = {}, tooltip = "", options = undefined }) {
		...
		if (value == null) {
			if (this.app.isNewUserSession) {
				...
				const localValue = localStorage["Quasar.Settings." + id];
				if (localValue) {
