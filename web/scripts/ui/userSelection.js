...
export class UserSelectionScreen {
	async show(users, user) {
		...
		const userSelection = document.getElementById("quasar-user-selection");
		...
		return new Promise((resolve) => {
			...
			const error = userSelection.getElementsByClassName("quasar-user-error")[0];
			const button = userSelection.getElementsByClassName("quasar-user-button-next")[0];
			...
			if (user) {
				const name = localStorage["Quasar.userName"];
				if (name) {
					input.value = name;
				}
			}
