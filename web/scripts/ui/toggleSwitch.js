...
export function toggleSwitch(name, items, { onChange } = {}) {
	...
	function updateSelected(index) {
		if (selectedIndex != null) {
			elements[selectedIndex].classList.remove("quasar-toggle-selected");
		}
		onChange?.({ item: items[index], prev: selectedIndex == null ? undefined : items[selectedIndex] });
		selectedIndex = index;
		elements[selectedIndex].classList.add("quasar-toggle-selected");
	}
	...
	const container = $el("div.quasar-toggle-switch", elements);
