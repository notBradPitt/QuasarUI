...
function seedWidget(node, inputName, inputData, app) {
	const seed = QuasarWidgets.INT(node, inputName, inputData, app);
...
function addMultilineWidget(node, name, opts, app) {
	...
	widget.inputEl.className = "quasar-multiline-input";
...
function isSlider(display, app) {
	if (app.ui.settings.getSettingValue("Quasar.DisableSliders")) {
		return "number"
	}
...
export const QuasarWidgets = {
	"INT:seed": seedWidget,
	"INT:noise_seed": seedWidget,
	FLOAT(node, inputName, inputData, app) {
		let widgetType = isSlider(inputData[1]["display"], app);
		let precision = app.ui.settings.getSettingValue("Quasar.FloatRoundingPrecision");
		let disable_rounding = app.ui.settings.getSettingValue("Quasar.DisableFloatRounding")
