import { $el, QuasarDialog } from "./ui.js";
...
$el("style", {
	textContent: `
        ...
        .quasar-logging-title {
            background: var(--tr-even-bg-color);
            font-weight: bold;
            margin-bottom: 5px;
            text-align: center;
        }
        .quasar-logging-log div {
...
class QuasarLoggingDialog extends QuasarDialog {
	constructor(logging) {
		super();
	...
	export() {
		...
		const a = $el("a", {
			href: url,
			download: `quasarui-logs-${Date.now()}.json`,
			style: { display: "none" },
			parent: document.body,
		});
		a.click();
	...
	show(entries) {
		...
		const headers = Object.values(cols).map((title) =>
			$el("div.quasar-logging-title", {
				textContent: title,
			})
		);
		const rows = entries.map((entry, i) => {
			return $el(
				"div.quasar-logging-log",
				{
...
export class QuasarLogging {
	...
	constructor(app) {
		...
		this.dialog = new QuasarLoggingDialog(this);
		this.addSetting();
		this.catchUnhandled();
		this.addInitData();
	}
	...
	addSetting() {
		const settingId = "Quasar.Logging.Enabled";
