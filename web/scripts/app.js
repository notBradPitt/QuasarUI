import { QuasarLogging } from "./logging.js";
import { QuasarWidgets, initWidgets } from "./widgets.js";
import { QuasarUI, $el } from "./ui.js";
...
import { QuasarAppMenu } from "./ui/menu/index.js";
import { getStorageValue, setStorageValue } from "./utils.js";
import { QuasarWorkflowManager } from "./workflows.js";
export const ANIM_PREVIEW_WIDGET = "$$quasar_animation_preview";
...
/**
 * @typedef {import("types/quasar").QuasarExtension} QuasarExtension
 */

export class QuasarApp {
	/**
	...
	constructor() {
		this.ui = new QuasarUI(this);
		this.logging = new QuasarLogging(this);
		this.workflowManager = new QuasarWorkflowManager(this);
		this.bodyTop = $el("div.quasarui-body-top", { parent: document.body });
		this.bodyLeft = $el("div.quasarui-body-left", { parent: document.body });
		this.bodyRight = $el("div.quasarui-body-right", { parent: document.body });
		this.bodyBottom = $el("div.quasarui-body-bottom", { parent: document.body });		  
		this.menu = new QuasarAppMenu(this);

		/**
		 * List of extensions that are registered with the app
		 * @type {QuasarExtension[]}
		 */
		this.extensions = [];
	...
	getPreviewFormatParam() {
		let preview_format = this.ui.settings.getSettingValue("Quasar.PreviewFormat");
		if(preview_format)
	...
	static onClipspaceEditorSave() {
		if(QuasarApp.clipspace_return_node) {
			QuasarApp.pasteFromClipspace(QuasarApp.clipspace_return_node);
		}
	}

	static onClipspaceEditorClosed() {
		QuasarApp.clipspace_return_node = null;
	}
	...
	static copyToClipspace(node) {
		...
		if(QuasarApp.clipspace_invalidate_handler) {
			QuasarApp.clipspace_invalidate_handler();
		}
	}
	...
	static pasteFromClipspace(node) {
		if(QuasarApp.clipspace) {
			// image paste
			if(QuasarApp.clipspace.imgs && node.imgs) {
				if(node.images && QuasarApp.clipspace.images) {
					if(QuasarApp.clipspace['img_paste_mode'] == 'selected') {
						node.images = [QuasarApp.clipspace.images[QuasarApp.clipspace['selectedIndex']]];
					}
					else {
						node.images = QuasarApp.clipspace.images;
					}
				...
				if(QuasarApp.clipspace.imgs) {
					...
					if(QuasarApp.clipspace['img_paste_mode'] == 'selected') {
						const img = new Image();
						img.src = QuasarApp.clipspace.imgs[QuasarApp.clipspace['selectedIndex']].src;
						node.imgs = [img];
						node.imageIndex = 0;
					}
					else {
						const imgs = [];
						for(let i=0; i<QuasarApp.clipspace.imgs.length; i++) {
							imgs[i] = new Image();
							imgs[i].src = QuasarApp.clipspace.imgs[i].src;
							node.imgs = imgs;
						}
					}
				}
			...
			if(node.widgets) {
				if(QuasarApp.clipspace.images) {
					const clip_image = QuasarApp.clipspace.images[QuasarApp.clipspace['selectedIndex']];
				...
				if(QuasarApp.clipspace.widgets) {
					QuasarApp.clipspace.widgets.forEach(({ type, name, value }) => {
	 ...
	 * @param {keyof QuasarExtension} method The extension callback to execute
	...
	#addRestoreWorkflowView() {
		...
		this.enableWorkflowViewRestore = this.ui.settings.addSetting({
			id: "Quasar.EnableWorkflowViewRestore",
		...
		node.prototype.getExtraMenuOptions = function (_, options) {
			...
			if (!QuasarApp.clipspace_return_node) {
				options.push({
					...
					callback: (obj) => {
						QuasarApp.copyToClipspace(this);
					},
				});

				if (QuasarApp.clipspace != null) {
					options.push({
						content: "Paste (Clipspace)",
						callback: () => {
							QuasarApp.pasteFromClipspace(this);
						},
					});
				}

				if (QuasarApp.isImageNode(this)) {
					options.push({
						content: "Open in MaskEditor",
						callback: (obj) => {
							QuasarApp.copyToClipspace(this);
							QuasarApp.clipspace_return_node = this;
							QuasarApp.open_maskeditor();
						},
					});
				}
			}
			...
			for (const item of items) {
				if (item.type.startsWith('image/')) {
					...
					if (this.canvas.current_node &&
						...
						QuasarApp.isImageNode(this.canvas.current_node)) {
	...
	async #loadExtensions() {
	    ...
	    this.logging.addEntry("Quasar.App", "debug", { Extensions: extensions });
	...
	async #migrateSettings() {
		...
		const settings = Object.keys(this.ui.settings).reduce((p, n) => {
			const v = localStorage[`Quasar.Settings.${n}`];
			if (v) {
				try {
					p[n] = JSON.parse(v);
				} catch (error) {}
	...
	async #setUser() {
		...
		let user = localStorage["Quasar.userId"];
		...
		if (!user || !users[user]) {
			...
			user = userId;
			localStorage["Quasar.userName"] = username;
			localStorage["Quasar.userId"] = user;
		...
		this.ui.settings.addSetting({
			id: "Quasar.SwitchUser",
			name: "Switch User",
			type: (name) => {
				let currentUser = localStorage["Quasar.userName"];
				if (currentUser) {
				...
				return $el("tr", [
					...
					$el("td", [
						$el("button", {
							...
							onclick: () => {
								delete localStorage["Quasar.userId"];
								delete localStorage["Quasar.userName"];
		...
		try {
			const loadWorkflow = async (json) => {
				if (json) {
					...
					const workflowName = getStorageValue("Quasar.PreviousWorkflow");
	...
	resizeCanvas() {
		// Limit minimal scale to 1, see https://github.com/quasaranonymous/QuasarUI/pull/845
	...
	async registerNodeDef(nodeId, nodeData) {
		...
		const node = Object.assign(
			...
			},
			{
				...
				quasarClass: nodeData.name,
				nodeData
			}
		);
		node.prototype.quasarClass = nodeData.name;
	...
	showMissingNodesError(missingNodeTypes, hasAddedNodes = true) {
		...
		this.logging.addEntry("Quasar.App", "warn", {
	 ...
	 * @param { import("./workflows.js").QuasarWorkflowInstance | null } workflow The workflow
	 */
	...
	async graphToPrompt(graph = this.graph, clean = true) {
		...
		for (const outerNode of graph.computeExecutionOrder(false)) {
			...
			for (const node of innerNodes) {
				...
				let node_data = {
					inputs,
					class_type: node.quasarClass,
				};

				if (this.ui.settings.getSettingValue("Quasar.DevMode")) {
	...
	/**
	 * Registers a Quasar web extension with the app
	 * @param {QuasarExtension} extension
	 */
...
export const app = new QuasarApp();
