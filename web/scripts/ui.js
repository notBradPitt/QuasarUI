...
import { QuasarDialog as _QuasarDialog } from "./ui/dialog.js";
import { toggleSwitch } from "./ui/toggleSwitch.js";
import { QuasarSettingsDialog } from "./ui/settings.js";

export const QuasarDialog = _QuasarDialog;
...
export class QuasarUI {
	constructor(app) {
		...
		this.settings = new QuasarSettingsDialog(app);
		...
		this.menuContainer = $el("div.quasar-menu", { parent: document.body }, [
			$el("div.drag-handle.quasar-menu-header", {
				style: {
					overflow: "hidden",
					position: "relative",
					width: "100%",
				...
				$el("span.drag-handle"),
				$el("span.quasar-menu-queue-size", { $: (q) => (this.queueSize = q) }),
				$el("div.quasar-menu-actions", [
					$el("button.quasar-settings-btn", {
						textContent: "⚙️",
						onclick: () => this.settings.show(),
					}),
					$el("button.quasar-close-menu-btn", {
						textContent: "\u00d7",
						onclick: () => {
			...
			$el("button", {
				id: "quasar-load-default-button", textContent: "Load Default", onclick: async () => {
			...
			$el("button", {
				id: "quasar-reset-view-button", textContent: "Reset View", onclick: async () => {
					app.resetView();
				}
			}),
		]);
		...
		const devMode = this.settings.addSetting({
			...
			type: "boolean",
			defaultValue: false,
			onChange: function(value) { document.getElementById("quasar-dev-save-api-button").style.display = value ? "flex" : "none"},
		});
