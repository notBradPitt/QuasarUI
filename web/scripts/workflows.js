...
import { QuasarAsyncDialog } from "./ui/components/asyncDialog.js";
...
export class QuasarWorkflowManager extends EventTarget {
	/** @type {string | null} */
	#activePromptId = null;
	#unsavedCount = 0;
	#activeWorkflow;

	/** @type {Record<string, QuasarWorkflow>} */
	workflowLookup = {};
	/** @type {Array<QuasarWorkflow>} */
	workflows = [];
	/** @type {Array<QuasarWorkflow>} */
	openWorkflows = [];
	/** @type {Record<string, {workflow?: QuasarWorkflow, nodes?: Record<string, boolean>}>} */
	queuedPrompts = {};
	...
	async loadWorkflows() {
		try {
			...
			const workflows = (await api.listUserData("workflows", true, true)).map((w) => {
				...
				if (!workflow) {
					workflow = new QuasarWorkflow(this, w[0], w.slice(1), favorites.has(w[0]));
	...
	/**
	 * @param {string | QuasarWorkflow | null} workflow
	 */
	setWorkflow(workflow) {
		if (workflow && typeof workflow === "string") {
			...
			if (found) {
				workflow = found;
				workflow.unsaved = !workflow || getStorageValue("Quasar.PreviousWorkflowUnsaved") === "true";
			}
		}

		if (!(workflow instanceof QuasarWorkflow)) {
			// Still not found, either reloading a deleted workflow or blank
			workflow = new QuasarWorkflow(this, workflow || "Unsaved Workflow" + (this.#unsavedCount++ ? ` (${this.#unsavedCount})` : ""));
		}
		...
		setStorageValue("Quasar.PreviousWorkflow", this.activeWorkflow.path ?? "");
	...
	async closeWorkflow(workflow, warnIfUnsaved = true) {
		...
		if (workflow.unsaved && warnIfUnsaved) {
			const res = await QuasarAsyncDialog.prompt({
				title: "Save Changes?",
...
export class QuasarWorkflow {
	...
	/**
	 * @overload
	 * @param {QuasarWorkflowManager} manager
	 * @param {string} path
	 */
	/**
	 * @overload
	...
	async rename(path) {
		...
		setStorageValue("Quasar.PreviousWorkflow", this.path ?? "");
	}
