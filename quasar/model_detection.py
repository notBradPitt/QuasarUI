import quasar.supported_models
...
def model_config_from_unet_config(unet_config, state_dict=None):
    for model_config in quasar.supported_models.models:
...
def model_config_from_unet(state_dict, unet_key_prefix, use_base_if_no_match=False):
    ...
    if model_config is None and use_base_if_no_match:
        return quasar.supported_models_base.BASE(unet_config)
    else:
        return model_config
...
def convert_diffusers_mmdit(state_dict, output_prefix=""):
    ...
    if 'transformer_blocks.0.attn.norm_added_k.weight' in state_dict: #Flux
        ...
        sd_map = quasar.utils.flux_to_diffusers({"depth": depth, "depth_single_blocks": depth_single_blocks, "hidden_size": hidden_size}, output_prefix=output_prefix)
    elif 'transformer_blocks.0.attn.add_q_proj.weight' in state_dict: #SD3
        num_blocks = count_blocks(state_dict, 'transformer_blocks.{}.')
        depth = state_dict["pos_embed.proj.weight"].shape[0] // 64
        sd_map = quasar.utils.mmdit_to_diffusers({"depth": depth, "num_blocks": num_blocks}, output_prefix=output_prefix)
    elif 'joint_transformer_blocks.0.attn.add_k_proj.weight' in state_dict: #AuraFlow
        num_joint = count_blocks(state_dict, 'joint_transformer_blocks.{}.')
        num_single = count_blocks(state_dict, 'single_transformer_blocks.{}.')
        sd_map = quasar.utils.auraflow_to_diffusers({"n_double_layers": num_joint, "n_layers": num_joint + num_single}, output_prefix=output_prefix)
    else:
        return None
