...
def calc_cond_uncond_batch(model, cond, uncond, x_in, timestep, model_options): #TODO: remove
    logging.warning("WARNING: The quasar.samplers.calc_cond_uncond_batch function is deprecated please use the calc_cond_batch one instead.")
...
def resolve_areas_and_cond_masks(conditions, h, w, device):
    logging.warning("WARNING: The quasar.samplers.resolve_areas_and_cond_masks function is deprecated please use the resolve_areas_and_cond_masks_multidim one instead.")
...
class CFGGuider:
    ...
    def inner_set_conds(self, conds):
        for k in conds:
            self.original_conds[k] = quasar.sampler_helpers.convert_cond(conds[k])
    ...
    def sample(self, noise, latent_image, sampler, sigmas, denoise_mask=None, callback=None, disable_pbar=False, seed=None):
        ...
        self.inner_model, self.conds, self.loaded_models = quasar.sampler_helpers.prepare_sampling(self.model_patcher, noise.shape, self.conds)
        device = self.model_patcher.load_device

        if denoise_mask is not None:
            denoise_mask = quasar.sampler_helpers.prepare_mask(denoise_mask, noise.shape, device)
        ...
        quasar.sampler_helpers.cleanup_models(self.conds, self.loaded_models)
