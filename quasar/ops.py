...
def cast_bias_weight(s, input=None, dtype=None, device=None):
    ...
    bias = None
    non_blocking = quasar.model_management.device_should_use_non_blocking(device)
...
class disable_weight_init:
    class Linear(torch.nn.Linear, CastWeightBiasOp):
        ...
        def forward_quasar_cast_weights(self, input):
            weight, bias = cast_bias_weight(self, input)
            return torch.nn.functional.linear(input, weight, bias)

        def forward(self, *args, **kwargs):
            if self.quasar_cast_weights:
                return self.forward_quasar_cast_weights(*args, **kwargs)
            else:
    ...
    class ConvTranspose2d(torch.nn.ConvTranspose2d, CastWeightBiasOp):
        ...
        def forward_quasar_cast_weights(self, input, output_size=None):
            num_spatial_dims = 2
    ...
    class Embedding(torch.nn.Embedding, CastWeightBiasOp):
        ...
        def forward_quasar_cast_weights(self, input, out_dtype=None):
