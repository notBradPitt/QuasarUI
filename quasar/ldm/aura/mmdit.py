...
class MMDiT(nn.Module):
    ...
    def patchify(self, x):
        B, C, H, W = x.size()
        x = quasar.ldm.common_dit.pad_to_patch_size(x, (self.patch_size, self.patch_size))
        x = x.view(
            B,
            C,
            (H + 1) // self.patch_size,
    ...
    def apply_pos_embeds(self, x, h, w):
        ...
        cur_dim = self.h_max
        pos_encoding = quasar.ops.cast_to_input(self.positional_encoding.reshape(1, cur_dim, cur_dim, -1), x)
    ...
    def forward(self, x, timestep, context, **kwargs):
        ...
        c = torch.cat([quasar.ops.cast_to_input(self.register_tokens, c).repeat(c.size(0), 1, 1), c], dim=1)
