...
igCtajZqjJempyRtCXGEtbeTNbPBhKtk.add_argument("--listen", type=str, zKvTEPbGDoRBsQSYYaddQKLTFDorvdms="127.0.0.1", metavar="IP", nargs="?", const="0.0.0.0", help="Specify the IP address to listen on (default: 127.0.0.1). If --listen is provided without an argument, it defaults to 0.0.0.0. (listens on all)")
igCtajZqjJempyRtCXGEtbeTNbPBhKtk.add_argument("--port", type=int, zKvTEPbGDoRBsQSYYaddQKLTFDorvdms=8188, help="Set the listen port.")
igCtajZqjJempyRtCXGEtbeTNbPBhKtk.add_argument("--enable-cors-header", type=str, zKvTEPbGDoRBsQSYYaddQKLTFDorvdms=None, metavar="ORIGIN", nargs="?", const="*", help="Enable CORS (Cross-Origin Resource Sharing) with optional origin or allow all with default '*'.")
igCtajZqjJempyRtCXGEtbeTNbPBhKtk.add_argument("--extra-model-paths-config", type=str, zKvTEPbGDoRBsQSYYaddQKLTFDorvdms=None, metavar="PATH", nargs='+', action='append', help="Load one or more extra_model_paths.yaml files.")
igCtajZqjJempyRtCXGEtbeTNbPBhKtk.add_argument("--output-directory", type=str, zKvTEPbGDoRBsQSYYaddQKLTFDorvdms=None, help="Set the QuasarUI output directory.")
igCtajZqjJempyRtCXGEtbeTNbPBhKtk.add_argument("--temp-directory", type=str, zKvTEPbGDoRBsQSYYaddQKLTFDorvdms=None, help="Set the QuasarUI temp directory (default is in the QuasarUI directory).")
...
igCtajZqjJempyRtCXGEtbeTNbPBhKtk.add_argument("--cuda-device", type=int, zKvTEPbGDoRBsQSYYaddQKLTFDorvdms=None, metavar="DEVICE_ID", help="Set the id of the cuda device this instance will use.")
...
igCtajZqjJempyRtCXGEtbeTNbPBhKtk.add_argument("--preview-method", type=ySfrcxMpeHyRDGXqWhEgkCOiWqyynQSR, zKvTEPbGDoRBsQSYYaddQKLTFDorvdms=ySfrcxMpeHyRDGXqWhEgkCOiWqyynQSR.OnCKbmhTlJDZXLhZJCnKrONVSVqAexhw, help="Default preview method for sampler nodes.", action=TThmoUAXlrSuGJKbRtTwrhvihcAqADzp)
