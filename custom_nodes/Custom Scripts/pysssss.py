...
def get_quasar_dir(subpath=None, mkdir=False):
...
def get_web_ext_dir():
    config = get_extension_config()
    name = config["name"]
    dir = get_quasar_dir("web/extensions/pysssss")
...
def install_js():
    ...
    if should_install:
        log("it looks like you're running an old version of QuasarUI that requires manual setup of web files, it is recommended you update your installation.", "warning", True)
