...
const style = `
#quasar-save-button, #quasar-load-button {
   position: relative;
   overflow: hidden;
}
	...
	constructor() {
		function addWorkflowMenu(type, getOptions) {
			return $el("div.pysssss-workflow-arrow", {
				parent: document.getElementById(`quasar-${type}-button`),
				onclick: (e) => {
					e.preventDefault();
					e.stopPropagation();
...
app.registerExtension({
	...
	async setup() {
		...
		const quasarDefault = "[QuasarUI Default]";
		const defaultWorkflow = app.ui.settings.addSetting({
			id: "pysssss.Workflows.Default",
			name: "🐍 Default Workflow",
			defaultValue: quasarDefault,
			type: "combo",
			options: (value) =>
				[quasarDefault, ...workflows.workflows].map((m) => ({
					value: m,
					text: m,
					selected: m === value,
				})),
		...
		document.getElementById("quasar-load-default-button").onclick = async function () {
			if (
				localStorage["Quasar.Settings.Quasar.ConfirmClear"] === "false" ||
				confirm(`Load default workflow (${defaultWorkflow.value})?`)
			) {
				if (defaultWorkflow.value === quasarDefault) {
					app.loadGraphData();
				} else {
