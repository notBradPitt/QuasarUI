...
function getType(node) {
	if (node.quasarClass === CHECKPOINT_LOADER) {
		return "checkpoints";
	}
	return "loras";
}
...
app.registerExtension({
	...
	init() {
		...
		const combo = QuasarWidgets["COMBO"];
		QuasarWidgets["COMBO"] = function (node, inputName, inputData) {
	...
	async beforeRegisterNodeDef(nodeType, nodeData, app) {
		const isCkpt = nodeType.quasarClass === CHECKPOINT_LOADER;
		const isLora = nodeType.quasarClass === LORA_LOADER;
		if (isCkpt || isLora) {
			const onAdded = nodeType.prototype.onAdded;
			nodeType.prototype.onAdded = function () {
				onAdded?.apply(this, arguments);
				const { widget: exampleList } = QuasarWidgets["COMBO"](this, "example", [[""]], app);
				...
				const getExample = async () => {
					...
					if (!exampleWidget) {
						exampleWidget = QuasarWidgets["STRING"](this, "prompt", ["STRING", { multiline: true }], app).widget;
			...
			if (this.imgs) {
				...
				if (img) {
					const nodes = app.graph._nodes.filter(
						(n) => n.quasarClass === LORA_LOADER || n.quasarClass === CHECKPOINT_LOADER
					);
					if (nodes.length) {
						options.unshift({
