...
def prepare_mask_ad(noise_mask, shape, device):
    ...
    noise_mask = quasar.utils.repeat_to_batch_size(noise_mask, shape[0])
...
class SampleSettings:
    def __init__(self, batch_offset: int=0, noise_type: str=None, seed_gen: str=None, seed_offset: int=0, noise_layers: 'NoiseLayerGroup'=None,
        ...
        self.seed_gen = seed_gen if seed_gen is not None else SeedNoiseGeneration.QUASAR
    ...
    def prepare_noise(self, seed: int, latents: Tensor, noise: Tensor, extra_seed_offset=0, extra_args:dict={}, force_create_noise=True):
        ...
        if isinstance(seed, Iterable):
            ...
            # replace initial noise if not batch_offset 0 or Quasar seed_gen or not NoiseType default
            if self.batch_offset != 0 or self.seed_offset != 0 or self.noise_type != NoiseLayerType.DEFAULT or self.seed_gen != SeedNoiseGeneration.QUASAR or force_create_noise:
...
class SeedNoiseGeneration:
    ...
    LIST = [QUASAR, AUTO1111]
    LIST_WITH_OVERRIDE = [USE_EXISTING, QUASAR, AUTO1111]

    @classmethod
    def create_noise(cls, seed: int, latents: Tensor, existing_seed_gen: str=QUASAR, seed_gen: str=USE_EXISTING, noise_type: str=NoiseLayerType.DEFAULT, batch_offset: int=0, extra_args: dict={}):
        # determine if should use existing type
        if seed_gen == cls.USE_EXISTING:
            seed_gen = existing_seed_gen
        if seed_gen == cls.QUASAR:
            return cls.create_noise_quasar(seed, latents, noise_type, batch_offset, extra_args)
    ...
    @staticmethod
    def create_noise_quasar(seed: int, latents: Tensor, noise_type: str=NoiseLayerType.DEFAULT, batch_offset: int=0, extra_args: dict={}):
        ...
        # quasar creates noise with a single seed for the entire shape of the latents batched tensor
        generator = torch.manual_seed(seed)
...
class FreeInitOptions(IterationOptions):
    ...
    def preprocess_latents(self, curr_i: int, model: ModelPatcher, latents: Tensor, noise: Tensor, cached_latents: Tensor, cached_noise: Tensor,
                           seed:int, sample_settings: SampleSettings, noise_extra_args: dict, sampler: quasar.samplers.KSampler=None, **kwargs):
