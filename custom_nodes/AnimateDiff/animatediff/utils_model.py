...
import folder_paths
from quasar.model_base import SD21UNCLIP, SDXL, BaseModel, SDXLRefiner, SVD_img2vid, model_sampling, ModelType
...
import quasar_extras.nodes_model_advanced
...
class ModelSamplingType:
    ...
    MAP = {
        ...
        LCM: quasar_extras.nodes_model_advanced.LCM,
    }
...
def factory_model_sampling_discrete_distilled(original_timesteps=50):
    class ModelSamplingDiscreteDistilledEvolved(quasar_extras.nodes_model_advanced.ModelSamplingDiscreteDistilled):
...
# based on code in quasar_extras/nodes_model_advanced.py
def evolved_model_sampling(model_config: ModelSamplingConfig, model_type: ModelType, alias: str, original_timesteps: int=None):
    ...
    if BetaSchedules.is_lcm(alias) or original_timesteps is not None:
        sampling_type = quasar_extras.nodes_model_advanced.LCM
        if original_timesteps is not None:
            ...
            sampling_base = quasar_extras.nodes_model_advanced.ModelSamplingDiscreteDistilled
...
class SigmaSchedule:
    def __init__(self, model_sampling: quasar.model_sampling.ModelSamplingDiscrete, model_type: ModelType):
    ...
    @staticmethod
    def apply_zsnr(new_model_sampling: quasar.model_sampling.ModelSamplingDiscrete):
        new_model_sampling.set_sigmas(quasar_extras.nodes_model_advanced.rescale_zero_terminal_snr_sigmas(new_model_sampling.sigmas))
