...
import quasar.ldm.modules.attention as attention
from quasar.ldm.modules.diffusionmodules import openaimodel
import quasar.model_management
import quasar.samplers
import quasar.sample
SAMPLE_FALLBACK = False
try:
    import quasar.sampler_helpers
except ImportError:
    SAMPLE_FALLBACK = True
import quasar.utils
from quasar.controlnet import ControlBase
import quasar.ops
...
# refer to forward_timestep_embed in quasar/ldm/modules/diffusionmodules/openaimodel.py
...
class FunctionInjectionHolder:
    ...
    def inject_functions(self, model: ModelPatcherAndInjector, params: InjectionParams):
        ...
        self.orig_groupnorm_manual_cast_forward = quasar.ops.manual_cast.GroupNorm.forward_quasar_cast_weights
        self.orig_sampling_function = quasar.samplers.sampling_function # used to support sliding context windows in samplers
        if SAMPLE_FALLBACK:  # for backwards compatibility, for now
            self.orig_get_additional_models = quasar.sample.get_additional_models
        else:
            self.orig_get_additional_models = quasar.sampler_helpers.get_additional_models
        ...
        if model.motion_models is not None:
            ...
            if not (info.mm_version == AnimateDiffVersion.V3 or
                ...
                quasar.ops.manual_cast.GroupNorm.forward_quasar_cast_weights = groupnorm_mm_factory(params, manual_cast=True)
            ...
            for motion_model in model.motion_models:
                if motion_model.model.img_encoder != None:
                    ...
                    break
            del info
        quasar.samplers.sampling_function = evolved_sampling_function
        if SAMPLE_FALLBACK:  # for backwards compatibility, for now
            quasar.sample.get_additional_models = get_additional_models_factory(self.orig_get_additional_models, model.motion_models)
        else:
            quasar.sampler_helpers.get_additional_models = get_additional_models_factory(self.orig_get_additional_models, model.motion_models)
    ...
    def restore_functions(self, model: ModelPatcherAndInjector):
        ...
        try:
            ...
            quasar.ops.manual_cast.GroupNorm.forward_quasar_cast_weights = self.orig_groupnorm_manual_cast_forward
            quasar.samplers.sampling_function = self.orig_sampling_function
            if SAMPLE_FALLBACK:  # for backwards compatibility, for now
                quasar.sample.get_additional_models = self.orig_get_additional_models
            else:
                quasar.sampler_helpers.get_additional_models = self.orig_get_additional_models
            ...
            logger.error("Encountered AttributeError while attempting to restore functions - likely, an error occured while trying " + \
                         "to save original functions before injection, and a more specific error was thrown by QuasarUI.")


def motion_sample_factory(orig_quasar_sample: Callable, is_custom: bool=False) -> Callable:
    def motion_sample(model: ModelPatcherAndInjector, noise: Tensor, *args, **kwargs):
        ...
        if type(model) != ModelPatcherAndInjector:
            return orig_quasar_sample(model, noise, *args, **kwargs)
        ...
        try:
            ...
            if iter_opts.need_sampler:
                ...
                if is_custom:
                    ...
                    if SAMPLE_FALLBACK:  # backwards compatibility, for now
                        # in older quasar, model needs to be loaded to get proper model_sampling to be used for sigmas
                        quasar.model_management.load_model_gpu(model)
                        iter_model = model.model
                    else:
                        iter_model = model
                    iter_kwargs[IterationOptions.SAMPLER] = quasar.samplers.KSampler(
            ...
            for curr_i in range(iter_opts.iterations):
                ...
                latents = wrap_function_to_inject_xformers_bug_info(orig_quasar_sample)(model, noise, *args, **kwargs)
            return latents
        finally:
            del latents
            del noise
...
def evolved_sampling_function(model, x, timestep, uncond, cond, cond_scale, model_options: dict={}, seed=None):
    ...
    if not ADGS.is_using_sliding_context():
        if hasattr(quasar.samplers, "calc_cond_batch"):
            cond_pred, uncond_pred = quasar.samplers.calc_cond_batch(model, [cond, uncond_], x, timestep, model_options)
        else:
            cond_pred, uncond_pred = quasar.samplers.calc_cond_uncond_batch(model, cond, uncond_, x, timestep, model_options)
    else:
        cond_pred, uncond_pred = sliding_calc_cond_uncond_batch(model, cond, uncond_, x, timestep, model_options)

    if hasattr(quasar.samplers, "cfg_function"):
        return quasar.samplers.cfg_function(model, cond_pred, uncond_pred, cond_scale, x, timestep, model_options, cond, uncond)
...
def sliding_calc_cond_uncond_batch(model, cond, uncond, x_in: Tensor, timestep, model_options):
                    ...
                    # look for control
                        ...
                        if hasattr(control_item, "sub_idxs"):
                            ...
                            raise ValueError(f"Control type {type(control_item).__name__} may not support required features for sliding context window; \
                                                use Control objects from Kosinkadink/QuasarUI-Advanced-ControlNet nodes, or make sure Advanced-ControlNet is updated.")
                        resized_actual_cond[key] = control_item
                        del control_item
                    elif isinstance(cond_item, dict):
                        new_cond_item = cond_item.copy()
                        # when in dictionary, look for tensors and CONDCrossAttn [quasar/conds.py] (has cond attr that is a tensor)
    ...
    for ctx_idxs in context_windows:
        ...
        if hasattr(quasar.samplers, "calc_cond_batch"):
            sub_cond_out, sub_uncond_out = quasar.samplers.calc_cond_batch(model, [sub_cond, sub_uncond], sub_x, sub_timestep, model_options)
        else:
            sub_cond_out, sub_uncond_out = quasar.samplers.calc_cond_uncond_batch(model, sub_cond, sub_uncond, sub_x, sub_timestep, model_options)
