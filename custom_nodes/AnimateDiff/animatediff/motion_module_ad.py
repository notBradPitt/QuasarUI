...
from quasar.ldm.modules.attention import FeedForward, SpatialTransformer
from quasar.model_patcher import ModelPatcher
from quasar.ldm.modules.diffusionmodules import openaimodel
from quasar.ldm.modules.diffusionmodules.openaimodel import SpatialTransformer
from quasar.controlnet import broadcast_image_to
from quasar.utils import repeat_to_batch_size
import quasar.ops
...
class AnimateDiffModel(nn.Module):
    def __init__(self, mm_state_dict: dict[str, Tensor], mm_info: AnimateDiffInfo):
        ...
        if quasar.model_management.unet_manual_cast(quasar.model_management.unet_dtype(), quasar.model_management.get_torch_device()) is None:
            ops = quasar.ops.disable_weight_init
        else:
            ops = quasar.ops.manual_cast
        self.ops = ops
...
def get_motion_module(in_channels, block_type: str, block_idx: int, module_idx: int,
                      temporal_pe, temporal_pe_max_len, ops=quasar.ops.disable_weight_init):
...
class EncoderOnlyTemporalModule(VanillaTemporalModule):
    ...
    @classmethod
    def create(cls, in_channels, block_type: str, block_idx: int, module_idx: int, ops=quasar.ops.disable_weight_init):
