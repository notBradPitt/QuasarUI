...
def slice_cnet(h, h_len, w, w_len, model:quasar.controlnet.ControlBase, img):
    if img is None:
...
def slices_T2I(h, h_len, w, w_len, model:quasar.controlnet.ControlBase, img):
...
def sample_common(model, add_noise, noise_seed, tile_width, tile_height, tiling_strategy, steps, cfg, sampler_name, scheduler, positive, negative, latent_image, start_at_step, end_at_step, return_with_leftover_noise, denoise=1.0, preview=False):
    ...
    if add_noise == "disable":
        ...
        noise = quasar.sample.prepare_noise(samples, noise_seed, skip)

    if noise_mask is not None:
        noise_mask = quasar.sample.prepare_mask(noise_mask, noise.shape, device='cpu')
    ...
    real_model = None
    modelPatches, inference_memory = quasar.sample.get_additional_models(positive, negative, model.model_dtype())
    quasar.model_management.load_models_gpu([model] + modelPatches, quasar.model_management.batch_area_memory(noise.shape[0] * noise.shape[2] * noise.shape[3]) + inference_memory)
    #quasar.model_management.load_model_gpu(model)
    real_model = model.model

    #models = quasar.sample.load_additional_models(positive, negative, model.model_dtype())

    sampler = quasar.samplers.KSampler(real_model, steps=steps, device=device, sampler=sampler_name, scheduler=scheduler, denoise=denoise, model_options=model.model_options)
    ...
    #cnets
    cnets =  quasar.sample.get_models_from_cond(positive, 'control') + quasar.sample.get_models_from_cond(negative, 'control')
    cnets = [m for m in cnets if isinstance(m, quasar.controlnet.ControlNet)]
    ...
    #T2I
    T2Is =  quasar.sample.get_models_from_cond(positive, 'control') + quasar.sample.get_models_from_cond(negative, 'control')
    T2Is = [m for m in T2Is if isinstance(m, quasar.controlnet.T2IAdapter)]
    ...
    spatial_conds_pos = [
        (c[1]['area'] if 'area' in c[1] else None, 
            quasar.sample.prepare_mask(c[1]['mask'], shape, device) if 'mask' in c[1] else None)
        for c in positive
    ]
    spatial_conds_neg = [
        (c[1]['area'] if 'area' in c[1] else None, 
    ...
    positive_copy = quasar.sample.broadcast_cond(positive, shape[0], device)
    negative_copy = quasar.sample.broadcast_cond(negative, shape[0], device)
    ...
    with tqdm(total=total_steps) as pbar_tqdm:
        pbar = quasar.utils.ProgressBar(total_steps)
    ...
    quasar.sample.cleanup_additional_models(modelPatches)
