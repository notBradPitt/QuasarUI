...
// Some fragments of this code are from https://github.com/LucianoCirino/efficiency-nodes-quasarui

function inpaintCropAndStitchHandler(node) {
    if (node.quasarClass == "InpaintCrop") {
    ...
    } else if (node.quasarClass == "InpaintExtendOutpaint") {
    ...
    } else if (node.quasarClass == "InpaintResize") {
        toggleWidget(node, findWidgetByName(node, "min_width"));
        toggleWidget(node, findWidgetByName(node, "min_height"));
        toggleWidget(node, findWidgetByName(node, "rescale_factor"));
