...
def rescale(samples, width, height, algorithm):
    if algorithm == "nearest":
        ...
        return quasar.utils.bislerp(samples, width, height)
    return samples

class InpaintCrop:
    """
    QuasarUI-InpaintCropAndStitch
    https://github.com/lquesada/QuasarUI-InpaintCropAndStitch
    ...
    # Parts of this function are from KJNodes: https://github.com/kijai/QuasarUI-KJNodes
...
class InpaintStitch:
    ...
    # This function is from quasar_extras: https://github.com/quasaranonymous/QuasarUI
    def composite(self, destination, source, x, y, mask=None, multiplier=8, resize_source=False):
        source = source.to(destination.device)
        if resize_source:
            source = torch.nn.functional.interpolate(source, size=(destination.shape[2], destination.shape[3]), mode="bilinear")

        source = quasar.utils.repeat_to_batch_size(source, destination.shape[0])
        ...
        if mask is None:
            ...
            mask = quasar.utils.repeat_to_batch_size(mask, source.shape[0])
