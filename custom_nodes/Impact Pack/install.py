...
def get_installed_packages():
    ...
    if pip_list is None:
        try:
            ...
            print(f"[QuasarUI-Manager] Failed to retrieve the information of installed pip packages.")
            return set()
...
try:
    ...
    print("### QuasarUI-Impact-Pack: Check dependencies")
        ...
        if os.path.exists(subpack_path):
            try:
                ...
                if platform.system() == 'Windows':
                    print(f"[QuasarUI-Impact-Pack] Please turn off QuasarUI and remove '{subpack_path}' and restart QuasarUI.")
                else:
    ...
    def remove_olds():
        ...
        old_py_path = os.path.join(custom_nodes_path, "quasarui-impact-pack.py")
    ...
    def ensure_pip_packages_last():
        ...
        try:
            ...
            try:
                ...
                print(f"[ERROR] QuasarUI-Impact-Pack: failed to install 'opencv-python'. Please, install manually.")
    ...
    def install():
        ...
        print(f"### QuasarUI-Impact-Pack: Updating subpack")
        try:
            import git
        except Exception:
        ...
        if os.path.exists(subpack_install_script):
            ...
            print(f"### QuasarUI-Impact-Pack: (Install Failed) Subpack\nFile not found: `{subpack_install_script}`")
        ...
        # Download model
        print("### QuasarUI-Impact-Pack: Check basic models")
        ...
        if not os.path.exists(onnx_path):
            print(f"### QuasarUI-Impact-Pack: onnx model directory created ({onnx_path})")
            os.mkdir(onnx_path)
    ...
    print("[ERROR] QuasarUI-Impact-Pack: Dependency installation has failed. Please install manually.")
    traceback.print_exc()
