...
try:
    from quasar_extras.nodes_custom_sampler import Noise_EmptyNoise, Noise_RandomNoise
    import node_helpers
except:
...
def ksampler(sampler_name, total_sigmas, extra_options={}, inpaint_options={}):
    if sampler_name == "dpmpp_sde":
        ...
        return quasar.samplers.sampler_object(sampler_name)
...
def sample_with_custom_noise(model, add_noise, noise_seed, cfg, positive, negative, sampler, sigmas, latent_image, noise=None, callback=None):
    ...
    if hasattr(quasar.sample, 'fix_empty_latent_channels'):
    ...
    if negative != 'NegativePlaceholder':
        ...
        # guider.set_cfg(cfg)
        samples = quasar.sample.sample_custom(model, noise, cfg, sampler, sigmas, positive, negative, latent_image,
