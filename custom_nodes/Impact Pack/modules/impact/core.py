...
import nodes
import quasar_extras.nodes_upscale_model as model_upscale
    ...
    def doit_ipadapter(self, model):
        ...
        if 'IPAdapterAdvanced' not in nodes.NODE_CLASS_MAPPINGS:
            if 'IPAdapterApply' in nodes.NODE_CLASS_MAPPINGS:
                raise Exception(f"[ERROR] 'QuasarUI IPAdapter Plus' is outdated.")

            utils.try_install_custom_node('https://github.com/cubiq/QuasarUI_IPAdapter_plus',
                                          "To use 'IPAdapterApplySEGS' node, 'QuasarUI IPAdapter Plus' extension is required.")
            raise Exception(f"[ERROR] To use IPAdapterApplySEGS, you need to install 'QuasarUI IPAdapter Plus'")
        ...
        if use_acn:
            if "ACN_AdvancedControlNetApply" in nodes.NODE_CLASS_MAPPINGS:
                ...
                utils.try_install_custom_node('https://github.com/BlenderNeko/QuasarUI_TiledKSampler',
                                              "To use 'ControlNetWrapper' for AnimateDiff, 'QuasarUI-Advanced-ControlNet' extension is required.")
    ...
    def apply(self, positive, negative, image, mask=None, use_acn=False):
        ...
        if use_acn:
            if "ACN_AdvancedControlNetApply" in nodes.NODE_CLASS_MAPPINGS:
                ...
                utils.try_install_custom_node('https://github.com/BlenderNeko/QuasarUI_TiledKSampler',
                                              "To use 'ControlNetAdvancedWrapper' for AnimateDiff, 'QuasarUI-Advanced-ControlNet' extension is required.")
                raise Exception("'ACN_AdvancedControlNetApply' node isn't installed.")
        else:
...
# REQUIREMENTS: BlenderNeko/QuasarUI_TiledKSampler
class TiledKSamplerWrapper:
    ...
    def sample(self, latent_image, hook=None):
        if "BNK_TiledKSampler" in nodes.NODE_CLASS_MAPPINGS:
            ...
            utils.try_install_custom_node('https://github.com/BlenderNeko/QuasarUI_TiledKSampler',
                                          "To use 'TiledKSamplerProvider', 'Tiled sampling for QuasarUI' extension is required.")
...
# REQUIREMENTS: biegert/QuasarUI-CLIPSeg
class BBoxDetectorBasedOnCLIPSeg:
    ...
    def detect_combined(self, image, bbox_threshold, bbox_dilation):
        if "CLIPSeg" in nodes.NODE_CLASS_MAPPINGS:
            ...
            utils.try_install_custom_node('https://github.com/biegert/QuasarUI-CLIPSeg/raw/main/custom_nodes/clipseg.py',
...
from quasar.cli_args import args, LatentPreviewMethod
import folder_paths
...
try:
    ...
    print(f"#########################################################################")
    print(f"[ERROR] QuasarUI-Impact-Pack: Please update QuasarUI to the latest version.")
    print(f"#########################################################################")
