...
async function uploadMask(filepath, formData) {
	...
	QuasarApp.clipspace.imgs[QuasarApp.clipspace['selectedIndex']].src = `view?filename=${filepath.filename}&type=${filepath.type}`;
...
class ImpactSamEditorDialog extends QuasarDialog {
	...
	show() {
		...
		const target_image_path = QuasarApp.clipspace.imgs[QuasarApp.clipspace['selectedIndex']].src;
	...
	async save(self) {
		...
		if(QuasarApp.clipspace.widgets) {
			...
			if(index >= 0)
				QuasarApp.clipspace.widgets[index].value = `${filename} [temp]`;
		}
