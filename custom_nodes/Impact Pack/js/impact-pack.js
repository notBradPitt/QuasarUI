...
// temporary implementation (copying from https://github.com/pythongosssss/QuasarUI-WD14-Tagger)
...
app.registerExtension({
	...
	loadedGraphNode(node, app) {
		if (node.quasarClass == "MaskPainter") {
	...
	nodeCreated(node, app) {
		if(node.quasarClass == "MaskPainter") {
			node.addWidget("button", "Edit mask", null, () => {
				QuasarApp.copyToClipspace(node);
				QuasarApp.clipspace_return_node = node;
				QuasarApp.open_maskeditor();
			});
		}

		switch(node.quasarClass) {
			case "ToDetailerPipe":
		...
		if(node.quasarClass == "ImpactSEGSLabelFilter" || node.quasarClass == "SEGSLabelFilterDetailerHookProvider") {
		...
		if(node.quasarClass == "UltralyticsDetectorProvider") {
		...
		if(
		node.quasarClass == "ImpactWildcardEncode" || node.quasarClass == "ImpactWildcardProcessor"
		|| node.quasarClass == "ToDetailerPipe" || node.quasarClass == "ToDetailerPipeSDXL"
		|| node.quasarClass == "EditDetailerPipe" || node.quasarClass == "EditDetailerPipeSDXL"
		|| node.quasarClass == "BasicPipeToDetailerPipe" || node.quasarClass == "BasicPipeToDetailerPipeSDXL") {
		...
		if(node.quasarClass == "ImpactWildcardProcessor" || node.quasarClass == "ImpactWildcardEncode") {
