...
import math
import quasar
from quasar.ldm.modules.diffusionmodules.util import extract_into_tensor, make_beta_schedule


def get_alphas_cumprod(model):
    """
    Same procedure as in quasar.model_sampling.ModelSamplingDiscrete.__init__
    """
...
def sigma_to_t(model, sigma):
    model_wrap = quasar.samplers.wrap_model(model.model)
...
def get_timesteps(model, steps, sampler, scheduler, denoise, device="cpu"):
    ...
    sampler = quasar.samplers.KSampler(
...
def undo_scaling(model, sigma, noise):
    ...
    sigma_data = quasar.samplers.wrap_model(model.model).inner_model.model_sampling.sigma_data
    return noise * (sigma ** 2 + sigma_data ** 2) ** 0.5  # Multiply to cancel out the division in quasar
