...
# QuasarUI-Impact-Pack

**Custom nodes pack for QuasarUI**
This custom node helps to conveniently enhance images through Detector, Detailer, Upscaler, Pipe, and more.


## NOTICE
* Selection weight syntax is changed(: -> ::) since V3.16. ([tutorial](https://github.com/ltdrdata/QuasarUI-extension-tutorials/blob/Main/QuasarUI-Impact-Pack/tutorial/ImpactWildcardProcessor.md))
* Starting from V3.6, requires latest version(Aug 8, 9ccc965) of QuasarUI.
...
* Starting from V3.0, nodes related to `mmdet` are optional nodes that are activated only based on the configuration settings.
  - Through QuasarUI-Impact-Subpack, you can utilize UltralyticsDetectorProvider to access various detection models.
...
* The permission error related to cv2 that occurred during the installation of Impact Pack has been patched in version 2.21.4. However, please note that the latest versions of QuasarUI and QuasarUI-Manager are required.
* The "PreviewBridge" feature may not function correctly on QuasarUI versions released before July 1, 2023.
* Attempting to load the "QuasarUI-Impact-Pack" on QuasarUI versions released before June 27, 2023, will result in a failure.
...
* UltralyticsDetectorProvider - Loads the Ultralystics model to provide SEGM_DETECTOR, BBOX_DETECTOR.
  ...
  - The various models available in UltralyticsDetectorProvider can be downloaded through **QuasarUI-Manager**.
...
* CLIPSegDetectorProvider - Wrapper for CLIPSeg to provide BBOX_DETECTOR.
  * You need to install the QuasarUI-CLIPSeg node extension.
...
* PixelTiledKSampleUpscalerProvider - It is similar to PixelKSampleUpscalerProvider, but it uses QuasarUI_TiledKSampler and Tiled VAE Decoder/Encoder to avoid GPU VRAM issues at high resolutions.
  * You need to install the [BlenderNeko/QuasarUI_TiledKSampler](https://github.com/BlenderNeko/QuasarUI_TiledKSampler) node extension.
...
* NoiseInjectionHookProvider - During each iteration of IterativeUpscale, noise is injected into the latent space while varying the strength according to a schedule.
  * You need to install the [BlenderNeko/QuasarUI_Noise](https://github.com/BlenderNeko/QuasarUI_Noise) node extension.
...
* TiledKSamplerProvider - QuasarUI_TiledKSampler is a wrapper that provides KSAMPLER.
  * You need to install the [BlenderNeko/QuasarUI_TiledKSampler](https://github.com/BlenderNeko/QuasarUI_TiledKSampler) node extension.
...
* LatentSender, LatentReceiver - The latent generated in LatentSender are automatically sent to the LatentReceiver with the same link_id.
  ...
  * Due to the current structure of QuasarUI, it is unable to distinguish between SDXL latent and SD1.5/SD2.1 latent. Therefore, it generates thumbnails by decoding them using the SD1.5 method.
...
* Switche nodes
  ...
  * Inversed Switch (Any) - In contrast to `Switch (Any)`, it takes a single input and outputs one of many. Due to QuasarUI's functional limitations, the value of `select` must be determined at the time of queuing a prompt, and while it can serve as a `Primitive Node` or `ImpactInt`, it cannot function properly when connected through other nodes. 
  * Guide
...
* Upon the initial execution, an `impact-pack.ini` file will be generated in the custom_nodes/QuasarUI-Impact-Pack directory.
```
[default]
...
1. `cd custom_nodes`
1. `git clone https://github.com/ltdrdata/QuasarUI-Impact-Pack.git`
3. `cd QuasarUI-Impact-Pack`
...
* You can use this colab notebook [colab notebook](https://colab.research.google.com/github/ltdrdata/QuasarUI-Impact-Pack/blob/Main/notebook/quasarui_colab_impact_pack.ipynb) to launch it. This notebook automatically downloads the impact pack to the custom_nodes directory, installs the tested dependencies, and runs it.
...
* Once you run the Impact Pack for the first time, an `impact-pack.ini` file will be automatically generated in the Impact Pack directory. You can modify this configuration file to customize the default behavior.
  ...
  * sam_editor_model: Specify the SAM model for the SAM editor.
    * You can download various SAM models using QuasarUI-Manager.
    * Path to SAM model: `QuasarUI/models/sams`
```
[default]
...
* QuasarUI/models/mmdets/bbox <= https://huggingface.co/dustysys/ddetailer/resolve/main/mmdet/bbox/mmdet_anime-face_yolov3.pth
* QuasarUI/models/mmdets/bbox <= https://raw.githubusercontent.com/Bing-su/dddetailer/master/config/mmdet_anime-face_yolov3.py
* QuasarUI/models/sams <= https://dl.fbaipublicfiles.com/segment_anything/sam_vit_b_01ec64.pth
...
![simple](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/images/simple.png)
...
* The FaceDetailer node is a combination of a Detector node for face detection and a Detailer node for image enhancement. See the [Advanced Tutorial](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/tutorial/advanced.md) for a more detailed explanation.
...
![simple-orig](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/images/simple-original.png) ![simple-refined](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/images/simple-refined.png)
...
![2pass-workflow-example](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/images/2pass-simple.png)
...
![2pass-example-original](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/images/2pass-original.png) ![2pass-example-middle](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/images/2pass-1pass.png) ![2pass-example-result](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/images/2pass-2pass.png)
...
![combination-workflow-example](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/images/combination.jpg)
![combination-example-original](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/images/combination-original.png) ![combination-example-refined](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/images/combination-refined.png)
...
#### 4. Iterative Upscale
![upscale-workflow-example](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/images/upscale-workflow.png)
...
![combination-example-original](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/images/upscale-original.png) ![combination-example-refined](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/images/upscale-3x.png)
...
![samdetector-menu](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/images/SAMDetector-menu.png)
...
![samdetector-dialog](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/images/SAMDetector-dialog.jpg)
...
![samdetector-result](https://github.com/ltdrdata/QuasarUI-extension-tutorials/raw/Main/QuasarUI-Impact-Pack/images/SAMDetector-result.jpg)

* When you execute using the reflected mask in the node, you can observe that the image and mask are displayed separately.


## Others Tutorials
* [QuasarUI-extension-tutorials/QuasarUI-Impact-Pack](https://github.com/ltdrdata/QuasarUI-extension-tutorials/tree/Main/QuasarUI-Impact-Pack) - You can find various tutorials and workflows on this page.
* [Advanced Tutorial](https://github.com/ltdrdata/QuasarUI-extension-tutorials/blob/Main/QuasarUI-Impact-Pack/tutorial/advanced.md)
* [SAM Application](https://github.com/ltdrdata/QuasarUI-extension-tutorials/blob/Main/QuasarUI-Impact-Pack/tutorial/sam.md)
* [PreviewBridge](https://github.com/ltdrdata/QuasarUI-extension-tutorials/blob/Main/QuasarUI-Impact-Pack/tutorial/previewbridge.md)
* [Mask Pointer](https://github.com/ltdrdata/QuasarUI-extension-tutorials/blob/Main/QuasarUI-Impact-Pack/tutorial/maskpointer.md)
* [ONNX Tutorial](https://github.com/ltdrdata/QuasarUI-extension-tutorials/blob/Main/QuasarUI-Impact-Pack/tutorial/ONNX.md)
* [CLIPSeg Tutorial](https://github.com/ltdrdata/QuasarUI-extension-tutorials/blob/Main/QuasarUI-Impact-Pack/tutorial/clipseg.md)
* [Extreme Highresolution Upscale](https://github.com/ltdrdata/QuasarUI-extension-tutorials/blob/Main/QuasarUI-Impact-Pack/tutorial/extreme-upscale.md)
* [TwoSamplersForMask](https://github.com/ltdrdata/QuasarUI-extension-tutorials/blob/Main/QuasarUI-Impact-Pack/tutorial/TwoSamplers.md)
* [TwoAdvancedSamplersForMask](https://github.com/ltdrdata/QuasarUI-extension-tutorials/blob/Main/QuasarUI-Impact-Pack/tutorial/TwoAdvancedSamplers.md)
* [Advanced Iterative Upscale: PK_HOOK](https://github.com/ltdrdata/QuasarUI-extension-tutorials/blob/Main/QuasarUI-Impact-Pack/tutorial/pk_hook.md)
* [Advanced Iterative Upscale: TwoSamplersForMask Upscale Provider](https://github.com/ltdrdata/QuasarUI-extension-tutorials/blob/Main/QuasarUI-Impact-Pack/tutorial/TwoSamplersUpscale.md)
* [Interactive SAM + PreviewBridge](https://github.com/ltdrdata/QuasarUI-extension-tutorials/blob/Main/QuasarUI-Impact-Pack/tutorial/sam_with_preview_bridge.md)
