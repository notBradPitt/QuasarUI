...
# ensure .js
print("### Loading: QuasarUI-Manager (V0.32)")

quasar_ui_required_revision = 1240
quasar_ui_revision = "Unknown"

quasar_path = os.path.dirname(folder_paths.__file__)
custom_nodes_path = os.path.join(quasar_path, 'custom_nodes')
js_path = os.path.join(quasar_path, "web", "extensions")

quasarui_manager_path = os.path.dirname(__file__)
local_db_model = os.path.join(quasarui_manager_path, "model-list.json")
local_db_alter = os.path.join(quasarui_manager_path, "alter-list.json")
local_db_custom_node_list = os.path.join(quasarui_manager_path, "custom-node-list.json")
local_db_extension_node_mappings = os.path.join(quasarui_manager_path, "extension-node-map.json")
git_script_path = os.path.join(os.path.dirname(__file__), "git_helper.py")

startup_script_path = os.path.join(quasarui_manager_path, "startup-scripts")
...
with open(os.path.join(quasarui_manager_path, 'channels.list'), 'r') as file:
    channels = file.read()
...
def read_config():
    try:
        ...
        if channel_url_list_is_valid:
            ...
            print(f"[WARN] QuasarUI-Manager: channel_url_list is invalid format")
            ch_url_list = ''
        ...
        return {
                    ...
                    'channel_url': default_conf['channel_url'] if 'channel_url' in default_conf else 'https://raw.githubusercontent.com/ltdrdata/QuasarUI-Manager/main',
        ...
        return {
            ...
            'badge_mode': 'none',
            'git_exe': '',
            'channel_url': 'https://raw.githubusercontent.com/ltdrdata/QuasarUI-Manager/main',
            'channel_url_list': ''
        }
...
def try_install_script(url, repo_path, install_cmd):
    int_quasarui_revision = 0

    if type(quasar_ui_revision) == int:
        int_quasarui_revision = quasar_ui_revision
    elif quasar_ui_revision.isdigit():
        int_quasarui_revision = int(quasar_ui_revision)

    if platform.system() == "Windows" and int_quasarui_revision >= quasar_ui_required_revision:
        ...
        print(f"\n## QuasarUI-Manager: EXECUTE => {install_cmd}")
        code = run_script(install_cmd, cwd=repo_path)

        if platform.system() == "Windows":
            try:
                if int(quasar_ui_revision) < quasar_ui_required_revision:
                    print("\n\n###################################################################")
                    print(f"[WARN] QuasarUI-Manager: Your QuasarUI version ({quasar_ui_revision}) is too old. Please update to the latest version.")
                    print(f"[WARN] The extension installation feature may not work properly in the current installed QuasarUI version on Windows environment.")
                    print("###################################################################\n\n")
            except:
                pass
...
def print_quasarui_version():
    global quasar_ui_revision
    try:
        repo = git.Repo(os.path.dirname(folder_paths.__file__))

        quasar_ui_revision = len(list(repo.iter_commits('HEAD')))
        current_branch = repo.active_branch.name
        git_hash = repo.head.commit.hexsha

        try:
            if int(quasar_ui_revision) < quasar_ui_required_revision:
                print(f"\n\n## [WARN] QuasarUI-Manager: Your QuasarUI version ({quasar_ui_revision}) is too old. Please update to the latest version. ##\n\n")
        except:
            pass

        if current_branch == "master":
            print(f"### QuasarUI Revision: {quasar_ui_revision} [{git_hash[:8]}] | Released on '{repo.head.commit.committed_datetime.date()}'")
        else:
            print(f"### QuasarUI Revision: {quasar_ui_revision} on '{current_branch}' [{git_hash[:8]}] | Released on '{repo.head.commit.committed_datetime.date()}'")
    except:
        print("### QuasarUI Revision: UNKNOWN (The currently installed QuasarUI is not a Git repository)")


print_quasarui_version()
...
def setup_js():
    import nodes
    js_dest_path = os.path.join(js_path, "quasarui-manager")

    if hasattr(nodes, "EXTENSION_WEB_DIRS"):
        if os.path.exists(js_dest_path):
            shutil.rmtree(js_dest_path)
    else:
        print(f"[WARN] QuasarUI-Manager: Your QuasarUI version is outdated. Please update to the latest version.")
        # setup js
        if not os.path.exists(js_dest_path):
            os.makedirs(js_dest_path)
        js_src_path = os.path.join(quasarui_manager_path, "js", "quasarui-manager.js")

        print(f"### QuasarUI-Manager: Copy .js from '{js_src_path}' to '{js_dest_path}'")
    ...
    if data['save_path'] != 'default':
        if '..' in data['save_path'] or data['save_path'].startswith('/'):
            ...
            if data['save_path'].startswith("custom_nodes"):
                base_model = os.path.join(quasar_path, data['save_path'])
            else:
...
async def install_custom_node(request):
    ...
    if res:
        print(f"After restarting QuasarUI, please refresh the browser.")
...
@server.PromptServer.instance.routes.get("/quasarui_manager/update_quasarui")
async def install_custom_node(request):
    print(f"Update QuasarUI")

    try:
        repo_path = os.path.dirname(folder_paths.__file__)

        if not os.path.exists(os.path.join(repo_path, '.git')):
            print(f"QuasarUI update fail: The installed QuasarUI does not have a Git repository.")
            return web.Response(status=400)
        ...
        if commit_hash != remote_commit_hash:
            git_pull(repo_path)
            execute_install_script("QuasarUI", repo_path)
            return web.Response(status=201)
        else:
            return web.Response(status=200)
    except Exception as e:
        print(f"QuasarUI update fail: {e}", file=sys.stderr)
        pass
