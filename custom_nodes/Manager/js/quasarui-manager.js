...
var update_quasarui_button = null;
...
async function install_checked_custom_node(grid_rows, target_i, caller, mode) {
	if(caller) {
        ...
        caller.updateMessage('<BR>To apply the installed/disabled/enabled custom node, please restart QuasarUI.');
	}
}

async function updateQuasarUI() {
    let prev_text = update_quasarui_button.innerText;
	update_quasarui_button.innerText = "Updating QuasarUI...";
	update_quasarui_button.disabled = true;
	update_quasarui_button.style.backgroundColor = "gray";

	try {
		const response = await api.fetchApi('/quasarui_manager/update_quasarui');

		if(response.status == 400) {
			app.ui.dialog.show('Failed to update QuasarUI.');
			app.ui.dialog.element.style.zIndex = 9999;
			return false;
		}

		if(response.status == 201) {
			app.ui.dialog.show('QuasarUI has been successfully updated.');
			app.ui.dialog.element.style.zIndex = 9999;
		}
		else {
			app.ui.dialog.show('QuasarUI is already up to date with the latest version.');
			app.ui.dialog.element.style.zIndex = 9999;
		}
		...
		app.ui.dialog.show(`Failed to update QuasarUI / ${exception}`);
		app.ui.dialog.element.style.zIndex = 9999;
		return false;
	}
	finally {
		update_quasarui_button.disabled = false;
		update_quasarui_button.innerText = prev_text;
	    update_quasarui_button.style.backgroundColor = "";
	}
}
...
async function updateAll(update_check_checkbox) {
	...
	update_all_button.innerText = "Updating all...(QuasarUI)";
	...
	try {
		...
		const response1 = await api.fetchApi('/quasarui_manager/update_quasarui');
		const response2 = await api.fetchApi(`/customnode/update_all?mode=${mode}`);

		if(response1.status != 200 && response2.status != 201) {
			app.ui.dialog.show('Failed to update QuasarUI or several extensions.<BR><BR>See terminal log.<BR>');
			app.ui.dialog.element.style.zIndex = 9999;
			return false;
		}
		if(response1.status == 201 || response2.status == 201) {
	        app.ui.dialog.show('QuasarUI and all extensions have been updated to the latest version.');
			app.ui.dialog.element.style.zIndex = 9999;
		}
		else {
			app.ui.dialog.show('QuasarUI and all extensions are already up-to-date with the latest versions.');
	        app.ui.dialog.element.style.zIndex = 9999;
        }
		...
		app.ui.dialog.show(`Failed to update QuasarUI or several extensions / ${exception}`);
		app.ui.dialog.element.style.zIndex = 9999;
		return false;
	}
	finally {
...
// -----
class CustomNodesInstaller extends QuasarDialog {
	...
	constructor() {
		super();
		this.search_keyword = '';
		this.element = $el("div.quasar-modal", { parent: document.body }, []);
	}
...
// -----
class AlternativesInstaller extends QuasarDialog {
	static instance = null;
...
// -----------
class ModelInstaller extends QuasarDialog {
	static instance = null;
...
// -----------
class ManagerMenuDialog extends QuasarDialog {
	static instance = null;
	...
	createButtons() {
		...
		update_quasarui_button =
				$el("button", {
					type: "button",
					textContent: "Update QuasarUI",
					onclick:
						() => updateQuasarUI()
				});
		...
		const res =
			[
				$el("tr.td", {width:"100%"}, [$el("font", {size:6, color:"white"}, [`QuasarUI Manager Menu`])]),
				$el("br", {}, []),
				...
				$el("button", {
					type: "button",
					textContent: "QuasarUI Community Manual",
					onclick: () => { window.open("https://blenderneko.github.io/QuasarUI-docs/", "quasarui-community-manual"); }
				}),
				$el("button", {
					type: "button",
					textContent: "QuasarUI Nodes Info",
					onclick: () => { window.open("https://ltdrdata.github.io/", "quasarui-node-info"); }
				}),
...
app.registerExtension({
	name: "Quasar.ManagerMenu",

	async setup() {
		const menu = document.querySelector(".quasar-menu");
