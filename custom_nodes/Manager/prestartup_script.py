...
try:
    # Logger setup
    if os.path.exists("quasarui.log"):
        if os.path.exists("quasarui.prev.log"):
            if os.path.exists("quasarui.prev2.log"):
                os.remove("quasarui.prev2.log")
            os.rename("quasarui.prev.log", "quasarui.prev2.log")
        os.rename("quasarui.log", "quasarui.prev.log")
    ...
    log_file = open("quasarui.log", "w", encoding="utf-8")
    ...
    print(f"[QuasarUI-Manager] Logging failed: {e}")


print("** QuasarUI start up time:", datetime.datetime.now())
...
if os.path.exists(script_list_path):
    print("\n#######################################################################")
    print("[QuasarUI-Manager] Starting dependency installation/(de)activation for the extension\n")
    ...
    with open(script_list_path, 'r') as file:
        for line in file:
            ...
            try:
                script = eval(line)
                print(f"\n## QuasarUI-Manager: EXECUTE => {script[1:]}")
    ...
    print("\n[QuasarUI-Manager] Startup script completed.")
    print("#######################################################################\n")
