...
def slice_cnet(h, h_len, w, w_len, model:quasar.controlnet.ControlBase, img):
    ...
    if isinstance(model, quasar.controlnet.ControlLora):
...
def sample_common(model, add_noise, noise_seed, tile_width, tile_height, tiling_strategy, steps, cfg, sampler_name, scheduler, positive, negative, latent_image, start_at_step, end_at_step, return_with_leftover_noise, denoise=1.0, preview=False):
    ...
    conds0 = \
        {"positive": quasar.sampler_helpers.convert_cond(positive),
         "negative": quasar.sampler_helpers.convert_cond(negative)}

    conds = {}
    for k in conds0:
        conds[k] = list(map(lambda a: a.copy(), conds0[k]))

    modelPatches, inference_memory = quasar.sampler_helpers.get_additional_models(conds, model.model_dtype())

    quasar.model_management.load_models_gpu([model] + modelPatches, model.memory_required(noise.shape) + inference_memory)
    real_model = model.model

    sampler = quasar.samplers.KSampler(model, steps=steps, device=device, sampler=sampler_name, scheduler=scheduler, denoise=denoise, model_options=model.model_options)
    ...
    cnets = [x for x in cnets if isinstance(x, quasar.controlnet.ControlNet)]
    cnet_imgs = [
    ...
    T2Is = [x for x in T2Is if isinstance(x, quasar.controlnet.T2IAdapter)]
    T2I_imgs = [
        torch.nn.functional.interpolate(m.cond_hint_original, (shape[-2] * 8, shape[-1] * 8), mode='nearest-exact').to('cpu')
    ...
    quasar.sampler_helpers.cleanup_additional_models(modelPatches)
