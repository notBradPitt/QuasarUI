...
from quasar.utils import load_torch_file
...
"""
    ...
    def load(self, model, model_path):
        ...
        if not os.path.exists(model_full_path):
            ...
            #Patch QuasarUI's LoRA weight application to accept multi-channel inputs. Thanks @huchenlei
            try:
...
class ICLight:
    def extra_conds(self, **kwargs):
        ...
        if image.shape[1:] != noise.shape[1:]:
            image = quasar.utils.common_upscale(image.to(device), noise.shape[-1], noise.shape[-2], "bilinear", "center")

        image = quasar.utils.resize_to_batch_size(image, noise.shape[0])

        process_image_in = lambda image: image
        out['c_concat'] = quasar.conds.CONDNoiseShape(process_image_in(image))
    ...
    def encode(self, positive, negative, vae, foreground, multiplier, opt_background=None):
        ...
        if opt_background is not None:
            ...
            if samples_1.shape[1:] != samples_2.shape[1:]:
                samples_2 = quasar.utils.common_upscale(samples_2, samples_1.shape[-1], samples_1.shape[-2], "bilinear", "disabled")
...
class DetailTransfer:
    ...
    def process(self, target, source, mode, blur_sigma, blend_factor, mask=None):
        ...
        if target.shape[1:] != source.shape[1:]:
            source_tensor = quasar.utils.common_upscale(source_tensor, W, H, "bilinear", "disabled")
